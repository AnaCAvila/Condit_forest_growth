...
model_iter <- function(pars, data) {
  ...
  A = pars[1]-exp(pars[2]*data$ppt) #asymptote, based on rainfall
  k = pars[3]*data$temp^2+pars[4]*data$temp #growth parameter, based on temperature
  B = pars[5]+pars[6]*(1-exp(pars[7]*data$burn))^2  #delay, based on # of days from last burn to abandonment
  # Prediction of the model
  return(A * (1 - exp(-k*(data$t)))^B)
}


#####################################

data = data.frame(t = runif(109, 0, 30), #ages
                  burn = runif(109, 0, 300),
                  temp = condit$temperature,
                  ppt = condit$annual_ppt)

pars <- c(100, -0.001, -0.0000025, 0.00125, 2, 1, -0.01)
#see range of valid parameters.

data$R <- model_iter(pars, data) + rnorm(109,0,5)

plot(data$t, data$R, las = 1, xlab = "Days after abandonment", ylab = "Biomass")

### TEST AREA
pars <- c(100, -0.001, 0, 0.00125, 2, -1, -0.01)
plot(data$t, model_iter(pars, data), las = 1, xlab = "Days after abandonment", ylab = "Biomass")
...
mle_optim_LM <- function(data) {
  ...
  plot(data$t, data$R)
  curve(model_iter(pars, data), 0, 30, add = TRUE)
